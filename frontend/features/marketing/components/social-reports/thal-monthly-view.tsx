  ...
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-4">
        <SectionHeader number="01" title="Sumário Executivo" />
        <InsightCallout title="Contexto do período">
          Retomada consistente da produção de conteúdo orgânico com 9 publicações no feed (+200% vs. anterior) e 3 stories. Alcance orgânico cresceu +51,62% mesmo com redução no investimento pago.
        </InsightCallout>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <ReportKpiCard icon={<IconEye className="h-4 w-4" />} label="Alcance Orgânico" value={fmt(MONTHLY_SUMMARY.reachOrganic.value)} metric={MONTHLY_SUMMARY.reachOrganic} accent />
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <ReportKpiCard icon={<IconEye className="h-4 w-4" />} label="Visualizações" value={fmt(MONTHLY_SUMMARY.views.value)} metric={MONTHLY_SUMMARY.views} />
          ...
          <ReportKpiCard icon={<IconPointer className="h-4 w-4" />} label="Interações" value={fmt(MONTHLY_SUMMARY.interactions.value)} metric={MONTHLY_SUMMARY.interactions} />
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <ReportKpiCard icon={<IconPhoto className="h-4 w-4" />} label="Publicações" value={String(MONTHLY_FEED.overview.posts.value)} metric={MONTHLY_FEED.overview.posts} />
        ...
        <InsightCallout title="Destaque do período">
          Publicação de harmonia e lifestyle do AUMA com a maior taxa de engajamento: 5,56%. Conteúdos com apelo emocional e conexão com o empreendimento performam acima da média.
        </InsightCallout>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <ReportKpiCard icon={<IconVideo className="h-4 w-4" />} label="Publicados" value={String(MONTHLY_REELS.overview.published.value)} sub="estável vs. anterior" />
        ...
        <InsightCallout title="Análise de Reels">
          Compartilhamento cresceu +33,33%, indicando potencial de distribuição. Recomenda-se aumentar frequência para 2-3 Reels/mês com formatos mais dinâmicos.
        </InsightCallout>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <ReportKpiCard icon={<IconEye className="h-4 w-4" />} label="Visualizações" value={fmt(MONTHLY_STORIES.overview.views.value)} deltaLabel="Novo" />
          ...
          <ReportKpiCard icon={<IconPointer className="h-4 w-4" />} label="Interações" value={fmt(MONTHLY_STORIES.overview.interactions.value)} deltaLabel="Novo" />
        </div>
        ...
        <MetricTable
          ...
          columns={[
            ...
            { key: "views", label: "Visualiz.", align: "right", format: (v) => fmt(v as number) },
            { key: "reach", label: "Alcance", align: "right", format: (v) => fmt(v as number) },
            { key: "interactions", label: "Interações", align: "right" },
        ...
        <InsightCallout title="Análise de Stories">
          Retomada positiva. Story de 05/04 com retenção de 80,67% — acima da média de mercado. Manter frequência semanal de 3-5 stories.
        </InsightCallout>
      </section>

      {/* ── 05 Audiência ── */}
      <section className="space-y-4">
        <SectionHeader number="05" title="Audiência" />
      ...
      {/* ── 06 Conclusões ── */}
      <section className="space-y-4">
        <SectionHeader number="06" title="Conclusões e Recomendações" />
        <div className="grid gap-4 md:grid-cols-2">
          <Card className="border-emerald-200 dark:border-emerald-900/40">
            ...
            <CardContent>
              <ul className="space-y-2 text-sm text-muted-foreground">
                <li className="flex gap-2"><span className="text-emerald-500 shrink-0">+</span> Volume de feed triplicou (+200%) com ganho em alcance e interações orgânicas</li>
                <li className="flex gap-2"><span className="text-emerald-500 shrink-0">+</span> Alcance orgânico +51,62% — algoritmo distribuindo melhor</li>
                <li className="flex gap-2"><span className="text-emerald-500 shrink-0">+</span> Retenção Stories acima de 80%</li>
                <li className="flex gap-2"><span className="text-emerald-500 shrink-0">+</span> Taxa de engajamento 5,56% em post conceitual (média do setor ~2-3%)</li>
              </ul>
            </CardContent>
          </Card>
          <Card className="border-orange-200 dark:border-orange-900/40">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-[#E85102]">Prioridades próximo mês</CardTitle>
            </CardHeader>
            <CardContent>
              <ul className="space-y-2 text-sm text-muted-foreground">
                <li className="flex gap-2"><span className="text-[#E85102] shrink-0">→</span> Aumentar Reels para 2-3/mês: maior potencial de distribuição orgânica</li>
                <li className="flex gap-2"><span className="text-[#E85102] shrink-0">→</span> Stories com frequência mínima semanal</li>
                <li className="flex gap-2"><span className="text-[#E85102] shrink-0">→</span> Explorar carrosséis editoriais para salvamentos</li>
                <li className="flex gap-2"><span className="text-[#E85102] shrink-0">→</span> Impulsionar conteúdos orgânicos de alta performance</li>
              </ul>
            </CardContent>
          </Card>
        </div>
