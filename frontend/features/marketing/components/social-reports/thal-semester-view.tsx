  ...
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-4">
        <SectionHeader number="07" title="Sumario Executivo — 6 Meses" />
        <InsightCallout title="Visao do semestre">
          Crescimento exponencial: visualizacoes +736%, interacoes +867%, visitas ao perfil +114%. Estrategia combinada de midia paga e producao organica consolidou 5.172 seguidores com base solida em Curitiba.
        </InsightCallout>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <ReportKpiCard icon={<IconEye className="h-4 w-4" />} label="Visualizacoes Totais" value={fmt(SEMESTER_SUMMARY.views.value)} metric={SEMESTER_SUMMARY.views} accent />
          ...
          <ReportKpiCard icon={<IconPointer className="h-4 w-4" />} label="Interacoes Totais" value={fmt(SEMESTER_SUMMARY.interactions.value)} metric={SEMESTER_SUMMARY.interactions} />
        </div>
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <ReportKpiCard icon={<IconEye className="h-4 w-4" />} label="Alcance Organico (30d)" value={fmt(SEMESTER_SUMMARY.reachOrganic.value)} metric={SEMESTER_SUMMARY.reachOrganic} />
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <ReportKpiCard icon={<IconPhoto className="h-4 w-4" />} label="Publicacoes" value={String(SEMESTER_FEED.overview.posts.value)} metric={SEMESTER_FEED.overview.posts} />
          <ReportKpiCard icon={<IconPointer className="h-4 w-4" />} label="Interacoes" value={fmt(SEMESTER_FEED.overview.interactions.value)} metric={SEMESTER_FEED.overview.interactions} accent />
        ...
        <InsightCallout title="Qualidade vs. Quantidade">
          Volume de postagens caiu -24,19%, mas todas as metricas de engajamento explodiram: compartilhamentos +585%, salvamentos +487%, interacoes +468%. Curadoria melhorou substancialmente.
        </InsightCallout>

        <MetricTable
          title="Top 5 — Publicacoes de Maior Alcance"
        ...
        <InsightCallout title="Padrao dos top performers">
          Todos os 5 conteudos de maior alcance sao Reels com narrativas institucionais ou de produto. Conteudos que reforcam solidez, proposito e legado geram engajamento de ate 10,43%.
        </InsightCallout>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <ReportKpiCard icon={<IconPointer className="h-4 w-4" />} label="Interacoes" value={fmt(SEMESTER_REELS.overview.interactions.value)} metric={SEMESTER_REELS.overview.interactions} accent />
        ...
        <InsightCallout title="Reels: motor do semestre">
          Interacoes cresceram +736% e curtidas +881%. Os 348 compartilhamentos (+461%) sao o dado mais estrategico: compartilhamento e o sinal mais forte de distribuicao organica no Instagram.
        </InsightCallout>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <ReportKpiCard icon={<IconBrandInstagram className="h-4 w-4" />} label="Publicados" value={String(SEMESTER_STORIES.overview.published.value)} deltaLabel="Inauguracao" />
          <ReportKpiCard icon={<IconEye className="h-4 w-4" />} label="Visualizacoes" value={fmt(SEMESTER_STORIES.overview.views.value)} deltaLabel="Inauguracao" />
          <ReportKpiCard icon={<IconPointer className="h-4 w-4" />} label="Interacoes" value={fmt(SEMESTER_STORIES.overview.interactions.value)} deltaLabel="Inauguracao" />
          <ReportKpiCard icon={<IconShare className="h-4 w-4" />} label="Compartilhamentos" value={fmt(SEMESTER_STORIES.overview.shares.value)} deltaLabel="Inauguracao" />
        </div>

        <MetricTable
          title="Top 6 Stories — por Retencao"
        ...
        <InsightCallout title="Benchmark de retencao">
          Retencao acima de 92% e referencia de excelencia — a media do mercado fica entre 70-80%. Stories de novembro/2025 consistentemente acima de 92%.
        </InsightCallout>
      </section>
      ...
      <section className="space-y-4">
            ...
            <div className="rounded-lg border overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-muted/40">
                  <tr>
                    <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">Metrica</th>
                    ...
                    <th className="px-4 py-2.5 text-right font-medium text-muted-foreground">Variacao</th>
                  </tr>
                </thead>
        ...
        <InsightCallout title="Sintese comparativa">
          Com menos conteudo publicado, a Thal gerou 10x mais interacoes totais e 8x mais visualizacoes no semestre. Evolucao qualitativa da producao: conteudo mais estrategico, melhor direcao criativa.
        </InsightCallout>
      </section>

      {/* ── 12 Direcionamentos ── */}
      <section className="space-y-4">
        <SectionHeader number="12" title="Conclusoes e Direcionamentos Estrategicos" />
          ...
          <Card className="border-emerald-200 dark:border-emerald-900/40">
            ...
            <CardContent>
              <ul className="space-y-2 text-sm text-muted-foreground">
                <li className="flex gap-2"><span className="text-emerald-500 shrink-0">+</span> +867% em interacoes totais</li>
                <li className="flex gap-2"><span className="text-emerald-500 shrink-0">+</span> Reels com engajamento de ate 10,43%</li>
                <li className="flex gap-2"><span className="text-emerald-500 shrink-0">+</span> Stories inaugurados com retencao acima de 92%</li>
                <li className="flex gap-2"><span className="text-emerald-500 shrink-0">+</span> Menos publicacoes com maior qualidade e engajamento</li>
              </ul>
            </CardContent>
          </Card>
          <Card className="border-orange-200 dark:border-orange-900/40">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-[#E85102]">Prioridades proximo semestre</CardTitle>
            </CardHeader>
            <CardContent>
              <ul className="space-y-2 text-sm text-muted-foreground">
                <li className="flex gap-2"><span className="text-[#E85102] shrink-0">→</span> Consolidar Reels (min. 2-3/mes)</li>
                <li className="flex gap-2"><span className="text-[#E85102] shrink-0">→</span> Calendario de Stories semanal</li>
                <li className="flex gap-2"><span className="text-[#E85102] shrink-0">→</span> Serie de conteudo para o AUMA</li>
                <li className="flex gap-2"><span className="text-[#E85102] shrink-0">→</span> Otimizar midia paga alinhada a conteudos organicos de alta performance</li>
